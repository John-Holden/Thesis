...
However, future research could examine non-local interactions between $R_0$-valued patches, in principle, this
could permit the construction of a higher resolution $R_0$-map composed of `within' and `between' patch interactions\textemdash echoing 
...
\begin{figure}
    \centering
    \includegraphics[scale=0.35]{chapter6/figures/comparisnos-adb-map.pdf}
    \caption{Comparisons between the Forestry Commissions ADB survey data and an example $R_0$-map. 
    (a) Forestry Commission survey data on the spread of ADB collected yearly between 2012-2020, green-purple respectively.
    (b) An example $R_0$-map with infectivity $\beta^*=650$ for the model $\phi_1$-ga. All susceptible patches at $1\mathrm{km \times 1 km}$ resolution are grouped into clusters. 
    In (b), a number of notable features (1-5) reveal similarities/differences when compared to the Forestry Commission data.}
    \label{fig:adb-comparison}
\end{figure}

Figures \ref{fig:adb-comparison}(a-b) compare FC data, collected from 2012 (green) to 2020 (purple), against an example CCA-clustered $R_0$-map for the model $\phi_1$-ga.
In Figure \ref{fig:adb-comparison}(b) all susceptible patches at $1\mathrm{km \times 1 km}$ resolution are grouped into clusters.
A number of compelling landscape features\textemdash circled from 1-5 in Figure \ref{fig:adb-comparison}(b)\textemdash indicate some potential similarities/differences to observational data.

From (1-5), circled in Figure \ref{fig:adb-comparison}(b), similarities/differences include:
\begin{enumerate}
    \item A large insusceptible region (shown in white) appears missing from the $R_0$-map in (b), yet the same region reports infected ash trees inside panel (a) from green-blue.
    \item In Scotland, we can see a general correlation between predicted susceptible regions in (b) and FC observations in (a).
    \item In panel (b), northern Wales appears significantly more fragmented/patchy and insusceptible when compared to the same region in panel (a).
    \item An insusceptible region in central Wales appears in panel (b), whereas, infected ash trees are reported in the same approximate region in panel (a).
    \item A region near Plymouth appears appears insusceptible, the same approximate region appears to have no reports of ADB in (a). 
\end{enumerate}

Ultimately, validating accuracy in the $R_0$-map is challenging due to insufficient disease incidence data.
The FC report from 2012-2020, shown in Figure \ref{fig:adb-comparison}(a), is among the most comprehensive ADB surveys undertaken in GB. 
The survey is composed yearly, over a large number of $\mathrm{10km \times 10km}$ grids, and reports the presence of ADB.
Unfortunately, the presence survey contains no estimates for the number of ADB cases within each grid, meaning that comparisons against the raw $R_0$-map 
pixel values are untenable with FC data. Thus, a better method to corroborate the model against observational data is required to progress the framework.
