...
% Ash dieback is a highly seasonal disease \cite{bengtsson2014seasonal}

% ash dieback follows a complicated life-cycle, the subject of much research over the years \cite{..}, although it can be understood to have to phases an asexual phase and sexual phase. The asexual phase occurs when the pathogen infects, and amplifies, through the host, the sexual phase is where it disperses...
...
\subsection{Constructing a simplified model of ash dieback}
...
% LDD : abbreviation for long distance dispersal
% reference the common use of long-tailed dispersal kernels
% figure showing connectivity element change...


% exists in that shed/dead plant material remain fixed at the hosts spatial location. 
...
Our results support the call for more research to be undertaken into multi-scale dispersal, in stark contrast to the promise a single kernel can support both large and small scale spread \cite{https://doi.org/10.1111/jbi.13642} % This sentence is wrong, the cited paper makes an argument for the spatially-scaling up of dispersal kernels, which happens to still be useful paper to cite, re-phrase and re-frame accordingly.
