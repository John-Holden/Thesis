...
From an $R_0$-map, a simplified notion of epidemic-impact can be visualised through an '$R_0$-cluster' over the population of ash.
...
However, the inverse power law models required a larger domain size of $\mathrm{5km}\times \mathrm{5km}$ to prevent underestimating infectivity.
...
\subsection{Landscape-level inverse power law $R_0$-clustering}
...
Consequently, when spatially scaling up the inverse power law model over Great Britain, each pixel reflects a $\mathrm{5km \times 5km}$ patch of land.
...
Figure \ref{fig:inverse-power-law-clustering} presents how $R_0$-clusters behave for the inverse power law model.
...
A value of $\beta$ has not been determined from the $SEIR$ model of ADB, thus compelling a study over the space of $\beta$.
Figures \ref{fig:inverse-power-law-clustering}(b) and (d) describe how $R_0$-clusters scale over different values of infectivity and structuring elements.
...
As $\beta^*$ increases, growth of the largest $R_0$-cluster results because patches are more likely to become susceptible and therefore join onto other neighbouring clusters.
...
\subsection{Landscape-level $R_0$-clustering: varying map resolution}
...
Resolving $R_0$-maps to the highest resolution possible is desirable to apprehend more information into the nature of spatial heterogeneity in epidemic severity.
...
Figures \ref{fig:gaussian-clustering}(a-c) show the frequency distributions of cluster size at three different patch sizes, of lengths $\mathrm{5km,\ 3km,\ 1km}$ respectively, and $\beta^*=500$.
Although the same general trend remains\textemdash i.e. a small number of large clusters alongside more numerous small clusters\textemdash
decreasing the patch-size produces a more fragmented domain, indicated by the greater variety of cluster sizes.
Moreover, the largest ranked cluster toward the far-right of each distribution shrinks for successively smaller patch sizes.
However, decreasing the patch size brings frequency distributions between structuring elements into closer alignment.
...
Problematically, performing CCA and the resulting cluster-growth at different landscape-level resolutions can yield deviations in results.
At the most extreme point, at $\beta^*=500$, larger deviations can be seen between the size of rank-1 susceptible $R_0$-clusters, illustrated by the vertical arrow in Figure \ref{fig:gaussian-clustering}.
Figures \ref{fig:gaussian-clustering}(e-g) depict the visual difference between CCA at domain resolution $\mathrm{5km \times 5km, 3km\times 3km}$ and $\mathrm{1km\times 1km}$.
Witnessing a difference in clustering behaviour, as per CCA, by the choice of landscape-level resolution is not desirable and outlines an important limitation of the framework we have constructed.

Although, similar landscape-level features in the $R_0$-maps are witnessed for each domain-resolution considered, disparities exist between cluster-sizes at different landscape-level resolutions.
As the $R_0$-map is resolved to a small spatial-scales, connectivity within an $R_0$-cluster can depend on a very small number of `critically-connecting' above-threshold patches.
Whereas, a coarse-grained domain would smooth critically-connecting patches in the process of averaging the small number of surroundings patches. % get up-close figure
Differences are caused by the simplistic notion of connectivity within CCA that fails to scale with the domain resolution.
Connectivity within an $R_0$-cluster is defined by local, nearest-neighbour patches above the threshold witch fall within the Moore or Von-Neumann structuring elements.

\begin{landscape}
\begin{figure}
    \centering
    \includegraphics[scale=0.38]{chapter6/figures/fig6-ga-cluster-distribution.pdf}
    \caption{Susceptible $R_0$-clustering explored over different landscape-level resolutions, namely from $\mathrm{1km \times 1km}$ to $\mathrm{5km \times 5km}$ patch-sizes. (a-c) Cluster-size distributions are shown for three landscape-level domain resolutions and both Moore and Von-Neumann structuring elements. As domain resolution is increased to $\mathrm{1km \times 1km}$, clusters can be resolved to a finer scale and yield a set of clusters over more length-scales (d) Cluster sizes over infectivity $\beta^*$ and structuring elements, comparing behaviour at two pixel-sizes. At the most extreme point, artefacts of structuring elements in CCA cause a large divergence between cluster sizes, indicated by the vertical arrow at $\beta^*=500$. (e-g) Spatial interpretations of cluster-size deviations at different domain resolutions according to the Moore neighbourhood at $\beta^*=500$.}
