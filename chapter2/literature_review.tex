...
Mathematically modelling communicative disease spread in tree populations is interdisciplinary, 
made possible by the convergence of numerous scientific fields. Consequently, this chapter reviews
several prerequisite themes essential to acknowledge before modelling the spread of tree disease. 
    
Modelling tree disease aims to help design effective control policies and inform policymakers.
Well-informed policymakers can then help to maintain tree health in numerous rural, urban and 
commercial environments. Although myriad environmental, biological and anthropomorphic factors 
complicate the scientific understanding and thus effective disease control. As such, understanding
modern-day tree disease epidemics requires a holistic, interdisciplinary scientific approach. 

The review begins by narrating early historical developments in human and botanical epidemiology before presenting
a variety of present-day modelling frameworks. After introducing some mainstream plant-based epidemiological models, an examination
of dispersal, thresholds, and epidemic control follows naturally. In addition, different host data sources in Great Britain
are reviewed. Finally, the chapter ends with an ash dieback case study, reflecting the multi-faceted difficulties posed by a
a recent emergent epidemic.

\section{Historical perspectives}

Historically, plant pathology had no cross-fertilisation with mathematical epidemiology. 
Instead, both fields existed in different spheres where early plant pathologists focused
on the biological mechanism of pathogen growth. Although pioneering discoveries 
in mathematical epidemiology permitted a more quantitative treatment of botanical diseases. 
In particular, the seminal $SIR$ model of \cite{kermack-model} 
provided a firm foundation to examine plant-based epidemics mathematically. 

\subsection{Standard $SIR$}

The mathematical model put forward by \cite{kermack-model} involved three compartmentalised fields, 
susceptible $S(t)$, infected $I(t)$ and removed $R(t)$.
Here, each field models a closed population of size $N$, where $N = S(t) + I(t) + R(t)$. 
...
The coupled ODE system through Equations (\ref{eq:SIR-model1}-\ref{eq:SIR-model3}) rely on several assumptions including:
...
the existence of a critical epidemic threshold:
\begin{align}
    \label{eq:R0-SIR}
    & R_0 = \frac{\beta}{\mu}\\
...
Both quantities $R_0$ and $R_e$ describe an epidemic threshold, 
yet $R_e$ reflects the monotonically declining susceptible host population by including the ratio $S(t)/N$.
In particular, Equation \ref{eq:R0-effective} defines a critical threshold by the simple criterion:
...
The reader is directed towards \cite{weiss2013sir} for a more comprehensive mathematical proof of $SIR$ 
model thresholds.
...
in proportion to the amount of inoculum. As time passes and $I$ grows, $(1-I)$ approximates zero and
the systems plateaus as all susceptible tissue becomes infected;
this essential behaviour is shown in Figure \ref{fig:SIR-vs-plank}(b) over $10$ values
growth rates $r$.
From Equation \ref{van-plank}, a simple method to determine the the infected rate $r$ follows:
\begin{equation}
...
\end{equation}
where $I_t$ and $I_{t-p}$ describe the infectious tissue at times $t$ and $t-p$. 
...
from Equation \ref{eq:R0-effective}. Equation \ref{van-plank-incubation} describes a system where
...
which lead Plank to extend the Equation \ref{van-plank-incubation} to follow:
\begin{equation}
...
The $SIR$ model does not include an incubation period, and therefore differs from the delayed differential formulation in Equation \ref{van-plank-infectious-p}.
Nonetheless, the compartmentalised approach is easy to extend, leading to an $SEIR$ system:
\begin{align}
...
Both $n$ and $m$ represent an arbitrary number of exposed and infectious compartments:
\[
    S\rightarrow E_1 \rightarrow E_2 \rightarrow ... \rightarrow E_n \rightarrow I_1 \rightarrow I_2 \rightarrow ... I_m
\]
In the trivial case, the $SEIR$ model is recovered when $n=1$ and $m=1$. However, when $n,m \rightarrow \infty$,
...
Despite the sameness of both DDE and $SEIR$ formulations, the overarching theme of modern epidemiological models is compartmental,
owing to their increased flexibility and easier analysis.

\subsection{Progressive botanical epidemiology}

Hitherto, plant pathologists had been trained to research botanical disease mainly through laboratory experiments
of infected tissue under the influence of various environmental conditions \cite{doi:10.1146/annurev.py.01.090163.000245}.
This had utility and practicality for understanding microscopic biological interactions, however, experiments of this
nature over scales ranging from individual plants, fields through to regions were untenable. With a new epidemiological
insight, borrowing from different mathematical techniques, plant pathologists were now able to extend their research to
account for the large-scale impact of diseases. 
