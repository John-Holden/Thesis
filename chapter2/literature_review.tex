...
Species distribution models (SDMs), first developed in the $1990$s, have become fundamental in ecological and biogeographical inference studies.
...
Correlative SDMs relate widely available presence-only (or presence/absence) data to several predictor variable datasets.
Following this, a species distribution map can be predicted, albeit with some uncertainties and error.
Commonly used statistical methods include Regression (GLM, GAM, MARS) and Machine Learning (ANN, CART, RF).
The general correlative approach is reflected in Figure \ref{fig:sdm}; for a more in-depth review of SDM types, see \cite{SDM_1}.
...
In particular, robust (SDM-generated) spatial distributions of forest species are vital to inform policymakers in the face of climate change \cite{SDM_1}
The 
...
\subsubsection{Modelling abundance}
