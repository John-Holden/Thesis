...
That is, if the mean cluster size, defined by a `characteristic' cluster length $C_r$, is larger or comparable to $L$, 
percolation can occur despite the density residing in the sub-critical regime. %
...
a low tree density reduces the chance of spread between nearest neighbours, producing a more chaotic epidemic and disrupting the highly ordered wavefront (panels (a-c)) into a circular travelling wavefront (panels (d-f)).
Although, it is worth remarking that an alternate triangular or honeycomb lattice geometry would change the diamond-like pattern altogether.
...
Two metrics categorised behaviour in the model: the pathogens average radial velocity and a probability of percolation.
