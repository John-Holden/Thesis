...
The appropriate mathematical definitions of percolation will be defined here, then criticality and self-similarity in the model will be discussed. %
This will include a brief summary of critical phenomena and some universal behaviour within the model. %
Following this, a more intricate two-parameter, `stochastic percolation', model will be developed. %
Stochastic percolation will be defined with an extra parameter that captures the infectivity of a pathogen. %
Inclusion of this extra parameter is justified given the variability of the virulence a given pathogen can demonstrate over a landscape. %
% JS does virulence on a 2d domain  have units of a density?
The behaviour of the two-parameter model will be investigated here using  ensemble-averaged parameter-sweeps. %
The two parameter model will constitutes a simple lattice model (SLM) of disease dynamics. %
Here, ensemble-averaging methods and metrics are introduced and used throughout this thesis. % %JS Chapter?

Lastly, the results of early-warning signals (EWS), investigated by \cite{OROZCOFUENTES201912}, will be extended. %
Extensions will include the formation of an alternate lattice-configuration and a metric that permits clearer EWS to be detected. %
Additionally, EWS are generalised to a two-dimensional parameter-space of $\rho$ and $\beta$. %
Conducting a thorough sweep through the the extra infectivity parameter reveals that EWS are more easily detected for some parameter-combinations % JS so this should be explored Analytically
and highlights interesting and distinct behaviour in the model.
\newpage
...
This parameterintroduced a novel element of temporal stochasticity into the system %
