...
A plethora of challenges complicate an effective on-the-ground response. 
...
This Chapter introduces the reasons for modelling tree disease epidemics, and the essential interaction between modellers and policymakers.
...
and tree populations that may lack evolutionary defences to invasive species \cite{doi:10.1002/9781444329988.ch8}. 
...
Two epidemics currently underway in the UK include ash dieback (\textit{Hymenoscyphus fraxineus}) affecting European ash \textit{Fraxinus excelsior} (\acrshort{fe})
...
In particular, policymakers can lead a variety of control initiatives to impede the spread of disease \cite{Gilligan-disease-management}. 
Effective epidemic control strategies benefit tree populations at a reduced economic cost \cite{ash-dieback-costs, freer2017tree, boyd2013consequence, tyrvainen2005benefits}. 
Commonly, preventative strategies include, thinning host densities \cite{pietzsch2021effect, resiliency-density-reductions} and cultivating genetically diverse distributions \cite{doi:10.1094/PD-89-0969, genetic-heterogeneity, huang1980importance}.
...
pest threats to tree species \cite{metcalf1994introduction}. In Great Britain (\acrshort{gb}), Asian longhorn beetle
(\acrshort{alb}) \cite{haack2010managing}, and oak processionary moth \acrshort{opm} \cite{tomlinson2015managing} are two concerns
...
Currently, the oomycete \textit{Phytophthora ramorum} \cite{brasier2005phytophthora}, and Ash dieback (\acrshort{adb})
caused by the fungus \textit{Hymenoscyphus fraxineus} (\acrshort{hf})\cite{ash-dieback-costs, mitchell2014ash} are two pathogens
...
in an environment unaltered by human transportation, tree and plant species are widely acknowledged to co-evolve with pests and pathogens
in a `gene-for-gene' arms-race \cite{Thrall1735, dangl2001plant, flor1971current}. 

However, the introduction of a foreign pathogen can overwhelm a population that has no such immunity \cite{desprez2016evolutionary}. Two classic examples that shook
...
Effective border controls on plant imports are indispensable to stop the spread of disease.
...
how growers and traders can transport plant material between countries\footnote{In light of Brexit, the \acrshort{uk} now plans
...
In this scenario, three vital questions to consider are: 
A) How do we effectively identify an infected tree? 
B) Which infected trees are the best choices to fell? 
C) What is the risk that a large-scale epidemic will result from observations of diseased trees?
...
agree that the scale of response must equal the scale of epidemic \cite{control-scale-matching}. In any case, 
...
over time \cite{control-theory, control-theory-application}. A naive eradication strategy can be fine-tuned and optimised in many ways to increase
efficiency. One scheme involves ranking host targets according to the risk they pose to prioritise culling \cite{risk-potential-control}.
...
that also seek to optimise resource allocations. Surveillance aims to detect infected individuals and disease incidence,
generally requiring the collection and analysis of data on botanical epidemics \cite{surveillance-review}.
...
In part, Figure \ref{fig:modelling-and-policies} was informed by interviewing civil servants/researchers/policymakers at DEFRA/Fera (San hutton, York).}. Scientists in several disciplines\textemdash
...
(\acrshort{cca}) algorithm is used to visualise highly clustered regions of $R_0$-values. Examining $R_0$-map clustering as a function
