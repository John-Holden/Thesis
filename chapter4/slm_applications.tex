...
\begin{equation}
   ...
   \label{eq:COM}
\end{equation}
where $x_i(t)$ is the spatial location along the $x$ axis of the $i^{th}$ infected tree at 
...
Figure \ref{fig:ews-primer}(b) depicts the COM time-series corresponding to the simulation of Figure \ref{fig:ews-primer}(a). 
The time-series now looks different to those shown previously in Figure \ref{fig:vel_eff_rad_metric}(a) and allows for negative values.

Figure \ref{fig:ews-primer}(c) illustrates COM ensemble-averaged time-series $\langle v_{cm}\rangle$ for the three values of tree density.
The blue time-series begins to decrease around $t \in [800, 900]$, coinciding with the pathogen propagating to the domain edge.
Whereas, the green time series lies slightly above the percolation threshold and decays more gradually from $t=0$, due to a higher extinction probability. 
