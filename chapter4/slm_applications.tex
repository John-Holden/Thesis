...
Figures \ref{fig:ews-primer}(a-c) shows the `channel' domain over three time-steps for parameters above the threshold, $\rho=0.65$ and $\beta=0.50$.
...
As expected, studies have confirmed that site and bond percolation in two and three-dimensional cylinders depend critically on the aspect ratio \cite{sangare2009continuum, munson1991estimation}.
Therefore, to keep consistent with the results of chapter \ref{ch3:two-param-model} we choose an aspect ratio that approaches the percolation threshold for a two-dimensional square. In practice, a domain size of $(L_x, L_y) = (350, 50)$, was sufficient, as shown through Figures \ref{fig:ews-primer}(a-c). 
...
\subsection{Center of infection mass}
...
The findings of \cite{OROZCOFUENTES201912} were gathered by first producing a distribution of mean time-series velocities $\overline{v}(t)$ 
...
Although, assessing calculations based on the mean time-series variance i.e. $var\big(v(t) \big)$ revealed a clearer EWS.
