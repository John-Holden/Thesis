...
Given that the effects of LDD (beyond $5\mathrm{km}$) were neglected, the utility, efficiency and practicality of real-life implementation remain unproven.
Fungal spores in particular are thought to be able to disperse over large distances, e.g. from mainland Europe to Great Britain \cite{freer2017tree, wylder2018evidence}
...
It would be intuitive to expect efficient control on thin-tailed Gaussian kernels.
However, for fat-tailed dispersal kernels, the efficacy of between-patch control remains far less obvious and more subject to scrutiny.

In summary, three hypothetical outcomes can be expected from future research: 
\begin{enumerate}
    \item Between-patch control is insufficient because large numbers of secondary infections arise in $\rho_{target}$ from infected trees in $\rho_{source}$.
    \item The development of an outbreak in $\rho_{target}$ is slowed down by control measures in $\rho_{control}$, conditioned on the epidemic and control parameters.
    \item Epidemic containment between patches can be achieved, thereby preventing the pathogen from propagating to $\rho_{target}$.
\end{enumerate}
